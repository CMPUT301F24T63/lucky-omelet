...
public class User {
    ...
    // Setters (used to restore status from database)
    ...
    /**
     * Sends a custom message to a list of receivers. This can be used to send a notification to
     * all the people still in the waiting list that they have not been selected for the event.
     *
     * @param receivers A list of users who will receive the message
     * @param customMessage The content of the message
     * @param event The event that the message is related to
     */
    public void sendCustomMessage(ArrayList<User> receivers, String customMessage, Event event) {
        for (User receiver : receivers) {
            receiver.notificationList.add(new Notification(event, receiver, false, customMessage));
        }
    }
